...
def generate_and_save_keypair(folder):
    ...
    with open(os.path.join(folder, 'B_private_key.pem'), 'wb') as f:
        f.write(private_key_pem)
    # Save public key to a PEM file
    with open(os.path.join(folder, 'B_public_key.pem'), 'wb') as f:
...
def save_server_public_key(server_public_key):
    ...
    with open(os.path.join('client_b', 'server_public_key.pem'), 'wb') as f:
...
def main():
    ...
    private_key, public_key = generate_and_save_keypair('client_b')
    # Connect to server
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    client_socket.connect((HOST, PORT))

    # Exchange public key and client name with server
    server_public_key = exchange_public_key_and_name(client_socket, "B", public_key)
    # Save server's public key
